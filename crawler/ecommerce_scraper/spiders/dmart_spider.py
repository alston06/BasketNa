...
class DMartSpider(scrapy.Spider):
    ...
    def __init__(self, category=None, search_query=None, max_pages=10, *args, **kwargs):
        super(DMartSpider, self).__init__(*args, **kwargs)
        ...
        if self.search_query:
            self.start_urls = [
                f'https://www.dmart.in/search?q={self.search_query.replace(" ", "%20")}'
            ]
        elif self.category:
            self.start_urls = [
                f'https://www.dmart.in/{self.category}'
            ]
        else:
            # Default to grocery category
            self.start_urls = [
                'https://www.dmart.in/grocery'
            ]
    ...
    def parse_product(self, response):
        ...
        try:
            ...
            product_id_match = re.search(r'/product/([A-Z0-9-]+)', response.url)
            if product_id_match:
    ...
    def closed(self, reason):
        self.logger.info(f"DMart spider closed: {reason}")
