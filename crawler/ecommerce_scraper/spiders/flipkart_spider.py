...
class FlipkartSpider(scrapy.Spider):
    ...
    def __init__(self, category=None, search_query=None, max_pages=10, *args, **kwargs):
        ...
        if self.search_query:
            self.start_urls = [
                f'https://www.flipkart.com/search?q={self.search_query.replace(" ", "%20")}'
            ]
        elif self.category:
            self.start_urls = [
                f'https://www.flipkart.com/{self.category}'
            ]
        else:
    ...
    def parse(self, response):
        ...
        product_links = [link for link in product_links if '/p/' in link and 'pid=' in link]
    ...
    def parse_product(self, response):
        ...
        try:
            ...
            product_id_match = re.search(r'pid=([A-Z0-9]+)', response.url)
            if product_id_match:
                item['product_id'] = product_id_match.group(1)
            
            # Product name
            product_name = response.css('h1[class*="title"]::text').get()
            if not product_name:
                product_name = response.css('span[class*="title"]::text').get()
            if product_name:
    ...
    def closed(self, reason):
        self.logger.info(f"Flipkart spider closed: {reason}")
